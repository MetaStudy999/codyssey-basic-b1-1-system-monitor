...
| 구분 | Linux와 OS |
| 학습시간 | 40시간 |
| 원본 | `b1-1-mission.pdf` (8쪽) |

> 원본 PDF의 문장과 요구사항을 유지하면서 제목, 목록, 예시를 Markdown 구조로 정리한 문서입니다.

## 1. 미션 소개

서버 장애가 났을 때 로그가 없으면, 원인 분석은 '감'에 의존하게 됩니다. 실제 현업에서 이런 상황이 발생 하면 복구 시간이 수 배로 늘어나는 건 물론이고, 같은 장애가 반복됩니다. 권한 관리, 네트워크 보안, 로그 자동화까지 서버를 운영하는 엔지니어처럼 직접 설계합니다.
...
- **파일 위치/권한 정책**
  ...
  - cron 실행 계정: agent-admin (agent-admin은 agent-core에 포함되어 실행 가능해야함)
