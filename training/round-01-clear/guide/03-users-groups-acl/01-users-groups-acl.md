# B1-1 모듈 03 — STEP 05 사용자·그룹·접근 제어 목록(Access Control List, ACL)

> [← 모듈 03 목차](README.md) · [다음: 모듈 04 →](../04-agent-runtime/README.md) · [전체 입문자 가이드](../../BEGINNER-GUIDE.md)

<a id="step-05"></a>
## STEP 05 — 사용자·그룹·디렉터리·접근 제어 목록(Access Control List, ACL) 구성
...
- **접근 제어 목록(Access Control List, ACL)** — owner/group/others 기본 권한 외에 특정 사용자·그룹에 세밀한 권한을 추가하는 규칙입니다.
- **기본 접근 제어 목록(Default Access Control List, Default ACL)** — 디렉터리 안에 새로 만들어지는 파일·디렉터리가 상속받을 ACL 기준입니다.
- **접근 제어 목록 마스크(ACL Mask)** — named user/group와 group class에 실제로 허용되는 최대 권한 범위를 제한하는 값입니다.
